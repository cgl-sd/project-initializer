...
- `README.md`：本仓库说明，包含中文和英文两个可切换版本。
- `SKILL.md`：Project Initializer 技能定义和工作规则。
- `agents/openai.yaml`：技能在 OpenAI/Codex 环境中的展示与默认提示配置。

## 使用方式

将本仓库作为一个 Codex skill 安装或放入你的 Codex skills 目录后，可以在项目中请求：
...
- `README.md`: Repository documentation with switchable Chinese and English versions.
- `SKILL.md`: The Project Initializer skill definition and workflow rules.
- `agents/openai.yaml`: Display metadata and the default prompt for OpenAI/Codex environments.

## Usage

After installing this repository as a Codex skill, or placing it in your Codex skills directory, ask Codex to use it:
